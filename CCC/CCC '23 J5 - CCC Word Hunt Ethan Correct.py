    ...
    for s in range(column):
        for w in words:
            ...
            for k in range(len(w)):
                ...
                # print(graph[i+k+1][s+k-1], i+k+1, s+k-1)
                # down_down
                state_d_l_down = True
                for r in range(1, len(w) - k):
                    if i + k + r > row - 1 or s + k - r < 0:
                        state_d_l_down = False
                        break

                    if graph[i + k + r][s + k - r] != w[k + r]:
                        state_d_l_down = False
                        break
